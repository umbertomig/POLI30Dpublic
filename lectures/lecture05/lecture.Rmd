...
## Unit of Measurement\newline of the Diffs-in-Means Estimator
- Formula of the difference-in-means estimator?
    +  \textit{Average outcome} for the treatment group - \newline \textit{Average outcome} for the control group
...
\vspace{.1cm}
    + because the data come from a randomized experiment (where treatment was randomly assigned)
...
2. Open **inclass01.R** from within RStudio 
    + RStudio: File >> Open File 
\vspace{.2cm}

3. Run code from steps 1-3
    + use the \fu{setwd()} for your computer
...
- \pause You need to run the code all at once (not line by line)
- \pause Remember that R will ignore anything that follows the \color{gray}\# \color{black} sign, until the end of the line
- \pause What would have happened had we not added \fu{voting\$} in front of \fu{pressure} on the first line of code above?
...
- Whenever we create a new variable, we should make sure it was created correctly by looking at the first few observations of the dataframe again
    + What's the code to ask R to show the first observations of a dataframe? \pause
    ```{r}
    head(voting, 3) # shows first observations
    ```
...
***
- \fu{mean(voting\$voted)} computes the mean of *voted* for all the observations in the dataset (that is, for all the registered voters in the experiment)
...
\vspace{.1cm}
    ...
    + inside the square brackets, we specify the criteria of selection; for example, we can specify a logical test using the relational operator \fu{==}; only the observations for which the test is true will be extracted
...
***
- Compute the mean of *voted* for treatment and control groups
    ```{r}
...
***
- Now, we can compute the difference-in-means estimator:
    ```{r, collapse=TRUE, comment='##'}
...
+ *percentage points* because it is the result of subtracting two percentages
...
\begin{center}
\begin{minipage}{0.8\paperwidth}
\begin{mdframed}[backgroundcolor=white,linecolor=white, linewidth=0.05cm]
\vspace{.5cm}
\begin{center}
\begin{tabular}{l}
