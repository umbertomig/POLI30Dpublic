...
date: |
    | \large\textbf{Lecture 05 | Estimating Causal Effects \newline with Randomized Experiments}
fontsize: 12pt
output:
  beamer_presentation:
    highlight: tango
...
- Quizzes and Participation:
  + On Canvas. Check the board for the attendance check password!
...
- Then, we will learn how to \concept{MEASURE}\ the characteristics of an entire population from a sample of survey respondents
\vspace{-.1cm}
...
- \concept{Causal effects}\ refer to the cause-and-effect connection between two variables 
    + the \concept{treatment variable (X)}: variable whose change may produce a change in the outcome variable
    + the \concept{outcome variable (Y)}: variable that may change as a result of a change in the treatment variable
\vspace{.2cm}
...
***
- In the voting dataset we have three variables, \textit{birth}, \textit{message}, and \textit{voted}, and we aim to answer the research question: "Does social pressure increase the probability of turning out to vote?"
+ What is the treatment variable? 
    + \textit{message}: indicates whether register voter received the message inducing social pressure 
+ What is the outcome variable?
    + \textit{voted}: indicates whether register voter voted
\vspace{.2cm}

+ The causal relationship we are interested in is:
\vspace{.4cm}
\begin{center}$\textrm{\textit{message}} \rightarrow \textrm{\textit{voted}}$\end{center}
\vspace{-.1cm}
...
***
\begin{center}\includegraphics[height=.45\textheight]{../../images/fork2.png}\end{center}

- Do we ever observe both potential outcomes for the same individual at the exact same time under the same circumstances? 
    + No, we only observe what happens in reality: \concept{the factual outcome}
    + We can never observe what would have happened had we made different decisions: \concept{the counterfactual outcome}
\vspace{.2cm}


***
\begin{center}\includegraphics[height=.55\textheight]{../../images/counterfactual.png}\end{center}
...
- To accomplish this, we move away from individual-level effects and focus on the average causal effects across a group of individuals
\vspace{.2cm}

- The \concept{average causal effect}\ of the treatment \textit{X} on the outcome \textit{Y} (also known as the average treatment effect) is the average of all the individual causal effects of \textit{X} on \textit{Y} within a group
\vspace{.1cm}
...
\concept{Random treatment assignment}\ makes the treatment and control groups \textit{on average} identical to each other in all observed and unobserved pre-treatment characteristics
\end{mdframed}
...
- Only when the treatment and control groups are comparable does the diffs-in-means estimator produce a \textit{valid} estimate of the average treatment effect
\vspace{.2cm}
    + $\widehat{\textrm{average\_effect}} = \overline{Y_\text{treatment group}} - \overline{Y_\text{control group}}$
    + the "hat" on top of the name denotes that this is an estimate
\vspace{-1cm}
...
\vspace{.3cm}
    + why proportions and not averages?
        + because \textit{voted} is binary so the average of \textit{voted} should be interpreted as a proportion, not an average
...
* Next class:
  + Hands on! We are going to analyze the voting experiment dataset!
