...
- Then, we have to statistically control for them, so that we may claim that the two groups comparable.
\vspace{-1cm}

## Confounders or Confounding Variables

- A \concept{confounding variable}, or \concept{confounder}, is a variable that affects both:
    1. The likelihood to receive the treatment $X$, and
    2. The outcome $Y$
...
- Suppose we are interested in the average causal effect of attending a private school, as opposed to a public one, on SAT performance.
...
- Ice cream sales and shark attacks are highly correlated. 
  + Does this mean that eating ice cream increases the probability that a shark attacks you?
...
- If we have a lottery to determine who is going to attend the private school at random, then we break the wealth link.
...
- We must identify all potential confounding variables and statistically control for them using a multiple linear regression model.
...
- Let's return to a beloved example: *Does Social Pressure Affect Turnout?*
...
- One can compute the difference-in-means estimator directly or one can fit a simple linear regression model.
  + Same results!
...
\item When $X$ is the treatment variable, then $\widehat{\beta}_1$ is equivalent to the difference-in-means estimator and, thus, we interpret $\widehat{\beta}_1$ using causal language: It is the $\triangle \widehat{Y}$ \textit{caused by} $\triangle X{=}\textrm{1}$. This causal interpretation is valid if there are no confounding variables: the treatment and control groups are comparable.
\end{itemize}
\vspace{.2cm}
\end{mdframed}
