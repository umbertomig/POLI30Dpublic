...
- How to install R and R Studio in your computer.
...
- `str` is short for structure of the data:
...
Each of the variables have four positions (like four observations).
...
And if you don't need something anymore, you can remove it:
...
## Loading a CSV dataset from locale

- CSV stands for [Comma Separated Values](https://en.wikipedia.org/wiki/Comma-separated_values). It is a special way to organize data:
  + Each line corresponds to one observation
  + Within lines, information for each variable is separated by a comma.

- To load data from the locale (i.e., your computer), you need to find and change your working directory.

- The book explains more on that. We are not going to deal with these cases in here.

- We will frequently analyze data from **GitHub**.

## Loading a CSV dataset from GitHub

To load a CSV dataset from the GitHub:
...
Nice and neat description of the data.frame. You can try with other objects too!
