...
- Piazza forum: For some reason the link in the slides does not work well. Check with TAs / Professor for alternative link.
...
- R Markdown are a great way of combining text and code.
...
- Compiles into three formats: `HTML`, `PDF`, or `DOCX`. 
  ...
  + But you only need `HTML`! The `HTML` has also the advantage of being easy to customize.
...
- To get started with a R Markdown, go to \fu{File > New File > R Markdown...}.
...
- The power of R is the fact that many people around the world can build R code and share.

- People do that by building `packages`. Think of packages as the R supplements.

- If you go to the gym and take whey to grow muscle, your R does the same by download and running packages.
...
- R does not load packages automatically, because for each loaded package, you eat up a little of the computer's memory.

- So you need to ask R to load it. To load the tidyverse family we do:
...
- Now go ahead and click there! If you did not use the command before, it is going to install a few packages.

- Then it is going to ask to save the file. Save as `lab04.Rmd`, in a folder that you will remember later.

- Once done, you should go to the folder and see the `lab04.Rmd` and another file, `lab04.html`. 
  + The `HTML` is a file that you can open in your internet browser!
...
- `YAML`: My suggestion is keep it simple:
...
- We can do ordered and unordered lists. Ordered start with numbers. Unordered start with star (`*`) or dash (`-`). Lower levels are followed by `+`.
...
- So far we used R Markdown as a text processor. But the power comes from the fact that we can do coding.

- To do in-line coding we do:
...
- And if you want to run your code chunks, you can use the little green play button.
  + It is located in the right-hand side of each of your chunks.
...
2. Comment your code as if you were doing a R Script.
...
5. PDFs are great, DOCX also good, but HTML is the best for begginer R coders.

\vspace{0.5cm}

6. The internet is a great source for help. 
